...
class Communitymeeting(models.Model):
    """ 
    A community meeting.
    Restricted to staff only for now.
    """

    title = models.CharField(max_length=100)
    slug = models.SlugField(max_length=100)
    desc = MarkupField('Description', null=True, blank=True)
    date = models.DateField()

    location_address = models.CharField('Location address', max_length=100, null=True, blank=True)
    geometry = models.PointField('Location', geography=True, null=True, blank=True) # default SRS 4326
    objects = models.GeoManager()
    
    class Meta:
        verbose_name = _('Community Meeting')
        verbose_name_plural = _('Community Meetings')
        ordering = ['-date']
...
class Checkin(models.Model):
    """ 
    Check-in for existing or anonymous users at community meetings.
    Check-ins are created by staff only (at registration at community meetings.
    """

    communitymeeting = models.ForeignKey('Communitymeeting', verbose_name='Community Meeting')
    existing_user = models.ForeignKey(User, verbose_name='Existing User', on_delete=models.SET(get_sentinel_user), null=True, blank=True)

    station = models.ForeignKey(Station, verbose_name='Home Station Area', null=True, blank=True)
    geometry = models.PointField(geography=True, null=True, blank=True) # default SRS 4326
    objects = models.GeoManager()
