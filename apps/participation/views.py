...
from participation.models import Station, Line, Theme, Shareditem, Idea, Meetingnote, Newsarticle, Media, Data, Communitymeeting, Checkin
...
def meeting_map_page(request, **kwargs):
    """ 
    Renders a map showing a map of all checked-in participants 
    Defaults to the last meeting
    """
    stations = Station.objects.all()
    lines = get_greenline()

    try:
        meeting = Communitymeeting.objects.get(pk=kwargs['id'])
    except:
        # default to most recent meeting
        meeting = Communitymeeting.objects.all()[0]

    checkins = Checkin.objects.filter(communitymeeting=meeting)

    return render_to_response("participation/map_meetings.html", locals(), context_instance=RequestContext(request))
